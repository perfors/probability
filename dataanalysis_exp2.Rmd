...
ggsave("figures/exp2_posteriors_peak_unif_adj.png",plot=p,device="png",
...
pbeta
ggsave("figures/exp2_beta_histograms_adj.png",plot=pbeta,device="png",
...
pgamma <- de2p_fitgamma %>%
  ...
  geom_histogram(alpha=0.7,fill="darkgreen",colour="black",binwidth=1) + 
  theme_bw() +
  labs(title = "Distribution of \u03b3 only",x="\u03b3",y="# of people")

pgamma
ggsave("figures/exp2_gamma_histograms_adj.png",plot=pgamma,device="png",
       width=8,height=5, units="cm")

p <- grid.arrange(pbeta,pgamma,ncol=2)
ggsave("figures/exp2_combined_histograms_adj.png",plot=p,device="png",
...
p <- de2p_fittwo %>%
  ggplot(mapping=aes(x=beta,y=gamma)) +
  geom_point(alpha=0.6,size=3,colour="darkgreen") + 
  theme_bw() +
  labs(title = "\u03b2 vs \u03b3",x="\u03b2",y="\u03b3")

p
ggsave("figures/exp2_beta_gamma_fitted_values_adj.png",plot=p,device="png",
       width=16,height=6, units="cm")
```
...
ggsave("figures/exp2_beta_gamma_histograms_adj.png",plot=p,device="png",
...
p <- de2peak_fittwo_sum %>%
  ...
  #geom_line(mapping=aes(x=px,y=ly),linetype="dotted",colour="gray") +
  geom_line(mapping=aes(x=px,y=ppy),linetype="solid",colour="gray") +
...
ggsave("figures/exp2_beta_gamma_indiv_fits_peaked_adj.png",plot=p,device="png",
...
p <- de2unif_fittwo_sum %>%
  ...
  #geom_line(mapping=aes(x=ux,y=ly),linetype="dotted",colour="gray") +
  geom_line(mapping=aes(x=ux,y=ppy),linetype="solid",colour="gray") +
...
ggsave("figures/exp2_beta_gamma_indiv_fits_uniform_adj.png",plot=p,device="png",
       width=40,height=50, units="cm")
...
de2predspeak$condition <- "Pᴇᴀᴋᴇᴅ"
de2predsunif$condition <- "Uɴɪғᴏʀᴍ"
...
pcorr <- de2preds %>%
  ...
  geom_histogram(alpha=0.7,colour="black",binwidth=0.05,show.legend=FALSE) + 
  scale_fill_manual(values = c("darkgreen", "green3")) +
  facet_wrap(~condition,ncol=1) +
  theme_bw() +
...
ggsave("figures/exp2_beta_gamma_fits_adj.png",plot=p,device="png",
...
dagg <- read.csv(file=here("datafiles/exp2all_aggregate_fits_adj.csv")) %>% as_tibble()
...
ggsave("figures/exp2_beta_gamma_aggregate_fits_adj.png",plot=p,device="png",
