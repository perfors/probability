---
title: "Analyses of main data: Experiment 1"
...
ggsave("figures/exp1_posteriors_five_unlimited_adj.png",plot=p,device="png",
...
ggsave("figures/exp1_posteriors_all_adj.png",plot=p,device="png",
       width=16,height=6, units="cm")
```
...
p
ggsave("figures/exp1_priors_all_adj.png",plot=p,device="png",
...
pg <- de1_fitall %>%
  ggplot(mapping=aes(x=gamma,fill=section)) +
...
pd <- de1_fitall %>%
  ggplot(mapping=aes(x=delta,fill=section)) +
  geom_histogram(alpha=0.7,colour="black",binwidth=2.5,show.legend=FALSE) + 
  scale_fill_manual(values = c("dodgerblue4", "dodgerblue2")) + 
  facet_wrap(~section,ncol=1) +
  theme_bw() +
...
ggsave("figures/exp1_beta_gamma_delta_histograms_adj.png",plot=p,device="png",
...
pbeta
ggsave("figures/exp1_beta_histograms_adj.png",plot=pbeta,device="png",
       width=8,height=10, units="cm")

# percent less than 0.1
pl1f <- round(100*sum(de1_fitbeta$beta[de1_fitbeta$section=="five"]<0.1)/107,1)
pl1u <- round(100*sum(de1_fitbeta$beta[de1_fitbeta$section=="unlimited"]<0.1)/107,1)
# percent greater than 0.9
pg1f <- round(100*sum(de1_fitbeta$beta[de1_fitbeta$section=="five"]>0.9)/107,1)
pg1u <- round(100*sum(de1_fitbeta$beta[de1_fitbeta$section=="unlimited"]>0.9)/107,1)
```
...
pgamma
ggsave("figures/exp1_gamma_histograms_adj.png",plot=pgamma,device="png",
       width=8,height=10, units="cm")

p <- grid.arrange(pbeta,pgamma,ncol=2)
ggsave("figures/exp1_combined_histograms_adj.png",plot=p,device="png",
       width=16,height=10, units="cm")

# likelihood percent less than 1
pll1f <- round(100*sum(de1_fitgamma$gamma[de1_fitgamma$section=="five"]<1)/107,1)
pll1u <- round(100*sum(de1_fitgamma$gamma[de1_fitgamma$section=="unlimited"]<1)/107,1)
```
...
p <- de1_fittwo %>%
  ggplot(mapping=aes(x=beta,y=gamma,colour=section)) +
...
p
ggsave("figures/exp1_beta_gamma_fitted_values_adj.png",plot=p,device="png",
       width=16,height=6, units="cm")
```
...
ggsave("figures/exp1_beta_gamma_histograms_adj.png",plot=p,device="png",
       width=16,height=10, units="cm")

# percent less than 0.1
pl1f <- round(100*sum(de1_fittwo$beta[de1_fittwo$section=="Fɪᴠᴇ"]<0.1)/107,1)
pl1u <- round(100*sum(de1_fittwo$beta[de1_fittwo$section=="Uɴʟɪᴍɪᴛᴇᴅ"]<0.1)/107,1)
# percent greater than 0.9
pg1f <- round(100*sum(de1_fittwo$beta[de1_fittwo$section=="Fɪᴠᴇ"]>0.9)/107,1)
pg1u <- round(100*sum(de1_fittwo$beta[de1_fittwo$section=="Uɴʟɪᴍɪᴛᴇᴅ"]>0.9)/107,1)

# likelihood percent less than 1
pll1f <- round(100*sum(de1_fittwo$gamma[de1_fittwo$section=="Fɪᴠᴇ"]<1)/107,1)
pll1u <- round(100*sum(de1_fittwo$gamma[de1_fittwo$section=="Uɴʟɪᴍɪᴛᴇᴅ"]<1)/107,1)

```

For the priors: `r pl1f`% in Fɪᴠᴇ and `r pl1u`% in Uɴʟɪᴍɪᴛᴇᴅ had $\beta$ less than 0.1, and `r pg1f`% in Fɪᴠᴇ and `r pg1u`% in Uɴʟɪᴍɪᴛᴇᴅ had $\beta$ greater than 0.9. 
...
*blue line*: their reported posterior after five
...
y <- x^4*(1-x)
de1afive_long$py <- rep(y/sum(y),107)
...
ggsave("figures/exp1_beta_gamma_indiv_fits_five_adj.png",plot=p,device="png",
...
ggsave("figures/exp1_beta_gamma_indiv_fits_unlim_adj.png",
       plot=p,device="png",
       width=40,height=50, units="cm")
...
ggsave("figures/exp1_beta_gamma_fits_adj.png",plot=p,device="png",
...
dagg <- read.csv(file=here("datafiles/exp1all_aggregate_fits_adj.csv")) %>% as_tibble()
...
ggsave("figures/exp1_beta_gamma_aggregate_fits_adj.png",plot=p,device="png",
